const zh: Record<string, string> = {
  // 标题栏
  "titlebar.suggest": "AI 续写",
  "titlebar.format": "AI 整理",
  "titlebar.off": "AI 关闭",
  "titlebar.formatting": "✨ 正在整理格式...",
  "titlebar.noKey": "⚠️ 未配置 API Key",
  "titlebar.settings": "设置",
  "titlebar.dark": "切换暗色",
  "titlebar.light": "切换亮色",

  // 编辑器
  "editor.placeholder": "开始写作... 不用管格式，AI会帮你整理 ✨",
  "editor.loading": "加载编辑器...",
  "editor.sourceMode": "源码模式 - 编辑 md 标记",
  "editor.folderName": "文件夹名称",

  // 底栏
  "statusbar.saving": "保存中...",
  "statusbar.saved": "✓ 已保存",
  "statusbar.suggest": "续写",
  "statusbar.format": "整理",
  "statusbar.off": "关闭",
  "statusbar.formatAll": "✨ 整理全文",
  "statusbar.save": "💾 保存",

  // 侧边栏
  "sidebar.newNote": "+ 新建笔记",
  "sidebar.search": "搜索笔记...",
  "sidebar.newFolder": "+ 新建文件夹",
  "sidebar.subFolder": "子文件夹名称",
  "sidebar.noNotes": "还没有笔记",
  "sidebar.emptyTips": "点击上方按钮创建",

  // 文件夹名（预设）
  "folder.diary": "日记",
  "folder.project": "项目",
  "folder.meeting": "会议",
  "folder.reading": "读书笔记",
  "folder.essay": "随笔",

  // 空状态
  "empty.title": "选择一个笔记或创建新笔记开始",
  "empty.subtitle": "写的时候不用管格式，AI会自动帮你整理 ✨",

  // 设置
  "settings.title": "设置",
  "settings.llm": "LLM 配置",
  "settings.editor": "编辑器",
  "settings.storage": "存储",
  "settings.language": "语言",
  "settings.provider": "AI 提供商",
  "settings.apiKey": "API Key",
  "settings.apiKeyHint": "你的 Key 只存在本地，不会上传",
  "settings.model": "模型",
  "settings.baseUrl": "API 地址",
  "settings.fontSize": "正文字号",
  "settings.fontFamily": "正文字体",
  "settings.small": "小",
  "settings.medium": "中",
  "settings.large": "大",
  "settings.fontDefault": "系统默认",
  "settings.fontSerif": "衬线",
  "settings.fontMono": "等宽",
  "settings.noteDir": "笔记存储目录",
  "settings.selectDir": "选择...",
  "settings.dirHint": "修改后保存，笔记列表将重新加载",
  "settings.save": "保存",
  "settings.close": "关闭",
  "settings.zh": "中文",
  "settings.en": "English",
};

export default zh;
